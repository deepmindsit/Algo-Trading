...
async def strategy_2():
    try:
        ...
        setting = await db[dbSetting].find_one({'strategy_name': "SVSB"})
        if setting['is_live']:
            stocks = []
            selectedStocks = []
            ...
            if len(selectedStocks) > 0:
                if AppConstants.websocket is not None:
                    ...
                    await StrgyUtils.subscribeWs(AppConstants.websocket, selectedStocks)
                ...
                ##########################################################
                thread = Thread(target=asyncio.run, args=(startStrategy2(setting, timeFrame, True, sleepTime),),
                                daemon=True, name='strategy_2')
                thread.daemon = True
                thread.start()
        ...
        AppConstants.log.error(f"s2: start s2 | {ex}")
        return AppUtils.responseWithoutData(False, status.HTTP_200_OK, str(ex))


async def startStrategy2(setting, timeFrame, fromCron, fromTime):
    try:
        while True:
        ...
        AppConstants.log.error(f"s2: loop | {ex}")
            ...
            if strikeData['trade_status']:
                ...
                for index, stock in enumerate(strikeData['stocks']):
                    ...
                    if currentCandle is not None and len(currentCandle) > 0:
                        ...
                        ################################################################################
                        strikeData = await checkChangeFromCallPut(index, strikeKey, strikeData, currentCandle, bbData, cEMA)
                        if strikeData['OTM'] == int(stock['strike_price']) or strikeData['ITM'] == int(stock['strike_price']):
                            await checkEntries(currentCandle, stock, setting)
                            strikeData = RedisDB.getJson(key=strikeKey)
                        ...
                        if strikeData['is_take_entry'] and ((stock['option_type'] == "CE" and stock['strike_price'] == strikeData['ITM']) or (stock['option_type'] == "PE" and stock['strike_price'] == strikeData['OTM'])):
                            await StrgyUtils.updateStrikePrice(db, "SVSB", strikeKey, strikeData, stock, currentCandle)

            # time.sleep(1)
    except Exception as ex:
        AppConstants.log.error(f"s2: check start trade | {ex}")
...
async def checkChangeFromCallPut(index, strikeKey, strikeData, currentCandle, df, cEMA):
    try:
        ...
        if stock['option_type'] == strikeData['status'] and strikeData['OTM'] == int(stock['strike_price']) or strikeData['ITM'] == int(stock['strike_price']):
            ######################################################################################################################
            # SOF: CHANGE OF STRIKE PRICE SAY CE TO PE OR PE TO CE
            ######################################################################################################################
            #####################################################################################################
            # Condition 2 : IF MARKET BREAKS LOW OF CONFIRMATION CANDLE
            #####################################################################################################
            if stock['confirmation_candle'] is not None and currentCandle[3] < stock['confirmation_candle'][3]:
                stock['is_upper_band_break'] = False
                stock['is_lower_band_break'] = False
                ...
                if strikeData['status'] == "CE":
                    ...
                    print(
                        f"-----------CE -- PE Change : {currentCandle[0]} | {stock['stock_name']} | {strikeData['ITM']} / {strikeData['OTM']} -----")
                else:
                    strikeData['status'] = "CE"
                    print(
                        f"-----------PE -- CE Change : {currentCandle[0]} | {stock['stock_name']} | {strikeData['ITM']} / {strikeData['OTM']} -----")
                print('---------------------------')

            ######################################################################################################################
            # Condition 1: UPPER BAND BREAKS AND CLOSE INSIDE THE BAND AND DOESN'T TOUCH THE EMA THEN THE LOW THAT CANDLE IS TSL
            ######################################################################################################################
            if stock['is_upper_band_break'] and currentCandle[2] < upperBand and currentCandle[4] > cEMA:
        ...
        AppConstants.log.error(f"s2: CE - PE | {ex}")
...
async def checkConditionAndGetEntry(timeFrame, stock, setting, strikeKey, strikeData, currentCandle, bbData, cEMA):
    try:
        ...
        sKey = f"{AppConstants.strategy_2}{stock['option_type']}_status_{timeFrame}"
        ...
        ################################################################################
        elif currentCandle[2] > upperBand:
            if status is None:
                status = {
                    "status": "Above UB",
                    "previous_candle": None,
        ...
        if status is not None:
            ...
            if status['status'] == "Below LB":
                ...
                #################################################################################################################################
                if currentCandle[4] > lowerBand and currentCandle[2] < cEMA:
                    ################################################################################
                    ...
                    if abs(currentCandle[2] - currentCandle[3]) <= range and strikeData['is_take_entry']:
                        ...
                        if currentCandle[0] < marketEndTime and getEntry and strikeData['status'] == stock['option_type']:
                            entry = await StrgyUtils.calculateEntry(setting, strikeKey, stock, currentCandle[0], currentCandle[2], 1.11, currentCandle[3], False)
        ...
        AppConstants.log.error(f"s2: check entries | {ex}")
...
async def checkEntries(currentCandle, stock, setting):
    try:
        timeFrame = int(setting['time_frame'].replace("M", ""))
        entryKey = f"{AppConstants.strategy_2}entry_{timeFrame}"
        entryData = RedisDB.getJson(key=entryKey)
        ...
        if entryData is not None:
            ...
            if len(cIndex) > 0:
                for index in cIndex:
                    ...
                    if entry['status'] == 0:
                        ################################################################################################################
                        # Condition For TSL : close below Low [ Confirmation Candle ]
                        ################################################################################################################
                        if stock['confirmation_candle'] is not None:
                            if currentCandle[4] < stock['confirmation_candle'][3]:
                    ...
                    if entry['is_entry'] == False and entry['is_exit'] == False and strikeData['is_take_entry'] == False:
                        ...
                        if removeData:
                            ...
                            removeKey = f"{AppConstants.strategy_2}removed_{timeFrame}"
...
async def strategy_2_back_test():
    try:
        db = await AppUtils.openDb()
        dbFuture = AppDatabase.getName().model_future
        dbSetting = AppDatabase.getName().strategy_setting
        ...
        if setting['is_live']:
            ...
            if len(selectedStocks) > 0:
                RedisDB.setJson(key="ws_stocks", data=selectedStocks)
                thread = Thread(target=asyncio.run, args=(startStrategy2BT(setting, timeFrame, startTime, False),),
                                daemon=True, name='strategy_2')
                thread.daemon = True
                thread.start()
...
async def startStrategy2BT(setting, timeFrame, startTime, fromCron):
    try:
        while True:
